...
void function () {
  var $cache, axilla, cacheTemplate, fs, getTemplateReference, glob, Handlebars, isAbsolutePath, isPartial, Path, readAndCompileSync, readFileSync, removeBasePath, removeExtensions, removeUnderscore, render, renderPartial, utils;
  fs = require('fs');
  Path = require('path');
  glob = require('glob');
  ...
  axilla.render = render = function (path, viewObject, options) {
    var layout, template, templateHtml;
    if (null == options)
      options = {};
    if (!(null != (template = $cache.templates[path])))
      throw new Error('Unable to resolve template at ' + path);
    if (options.layout === false)
      return template.render(viewObject);
    layout = null != options.layout ? options.layout : axilla._defaultLayout;
    if (!(null != (layout = $cache.layouts[layout])))
      throw new Error('Unable to resolve layout at ' + layout);
    templateHtml = new Handlebars.SafeString(template.render(viewObject));
    return layout.render(utils.extend(viewObject, { 'yield': templateHtml }));
  };
  axilla._defaultLayout = null;
  axilla.setDefaultLayout = function (layout) {
    return axilla._defaultLayout = layout;
  };
  ...
  renderPartial = function (path, viewObject) {
    var partial;
    if (!(null != (partial = $cache.partials[path])))
  ...
  cacheTemplate = function (path, options) {
    $cache[null != options.cacheType ? options.cacheType : 'templates'][options.as] = {
      render: function () {
